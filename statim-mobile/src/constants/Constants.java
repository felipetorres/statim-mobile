package constants;

public class Constants {

	public static final String statim_filename = "statim";
	public static final String statim_registration_id = "old_registration_id";
	public static final String statim_itinerary = "last_address";
	public static final String sender_email = "felipe.gtorres@gmail.com";
	public static final String sender_id = "660118395277";
	public static final String server_url = "http://statim.herokuapp.com";
}
